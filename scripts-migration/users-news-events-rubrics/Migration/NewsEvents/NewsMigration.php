    ...
    */
    ...
    foreach ($articles as $article) {
      ...
      if (0 === $i % 50 || $i === $length -1) {
        ...
        $requeteInsertRedirection = 'INSERT INTO ' . $this->wpTablePrefix . 'slug_history (`post_id`, `url`)
        VALUES ' . implode(', ', $insert_redirection) . '
        ON DUPLICATE KEY UPDATE `post_id`=VALUES(`post_id`), `url`=VALUES(`url`);';
        try {
          $this->wpDbConnection->exec($requeteInsertRedirection);
        } catch(Exception $e) {
          echo "-- ECHEC " . __FUNCTION__ . " REQUÊTE: [$requeteInsertRedirection]" . PHP_EOL;

          throw new MigrationException($e->getMessage(), $e->getCode(), $requeteInsertRedirection, __FUNCTION__);
