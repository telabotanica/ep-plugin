...
use Migration\Users\BpPseudoBuilder ;
...
/*
  private function insertUserIntoWpUserTable($id, $login, $pwd, $nicename, $email, $url, $has_registered, $status, $display_name) {
    ...
    try {
      ...
      throw new MigrationException($e->getMessage(), $e->getCode(), $req, __FUNCTION__);
    }
...
*/
  ...
  private function insertUserIntoBpXprofileDataTable($utilisateur) {
    ...
    try {
      ...
      throw new MigrationException($e->getMessage(), $e->getCode(),
        $requete_pseudo_bp, __FUNCTION__);
    }
  ...
  private function fetchTelaDbUsers() {
    ...
    return $this->telaDbConnection->query($requeteUtilisateurs)->fetchAll(PDO::FETCH_ASSOC);
