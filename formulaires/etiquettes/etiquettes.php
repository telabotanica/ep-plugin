...
function gtags_header() {
	echo '<link rel="stylesheet" type="text/css" href="'.WP_PLUGIN_URL.'/tela-botanica/formulaires/etiquettes/etiquettes.css" media="screen" />'."\n";
}
...
function gtags_show_tags_in_add_form() {
	...
	$i = 0;
	foreach ( $tags as $tag ) {
		echo ' <span class="gtags-add highlight etiquette pointer" title="Ajouter le mot-clé ' . $tag->name .'">' . $tag->name .'</span>';
		++$i;
...
function gtags_show_tags_in_directory() {
	if ( get_option('gtags_show_in_dir_list') == 'true' && gtags_get_group_tags() ) {
		?><div class="gtags-item-desc gtags-item-tags absolute"><?php echo gtags_make_tags_for_group() ?></div><?php
	}
}
...
function gtags_show_tags_in_header( $description ) {
	...
	if ( $gtags_single_header == 'true' || !$gtags_single_header )  { // default is to show
		if ( gtags_get_group_tags() ) {
			$description .= '<div class="center gtags-header">' . gtags_make_tags_for_group().'</div>';
		}
	}
